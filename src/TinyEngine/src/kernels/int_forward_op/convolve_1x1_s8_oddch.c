...
extern uint8_t toExecTable[(IMG_SIZE / 8) * NUM_NEURONS];
...
tinyengine_status convolve_1x1_s8_oddch(const q7_t *input, const uint16_t input_x,
    ...
    const uint8_t *neuronToExecTable = &toExecTable[0];
    trigger_high();
    NOP4;NOP4;NOP4;NOP4;
    NOP4;NOP4;NOP4;NOP4;
    /* Computing 1 neuron at a time */
      ...
      uint8_t input_set = 0;
        ...
        /* This assembler snippet processes 8 pixels at a time. */
        __asm__(
          ...
          /* Outputs  */ :[sum] "+r" (sum)
          /* Inputs   */ : [input_set] "r" (input_set), [ker_a] "m" (ker_a), [ip_as_col] "m" (ip), [IaPAM] "g" (IaPAM), [toExecTable] "m" (neuronToExecTable)
          /* Clobbers */ : "r0", "r1", "r2");
